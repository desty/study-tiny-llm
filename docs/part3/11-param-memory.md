...
!!! quote "전제"
    [Ch 10 nanoGPT](10-nanogpt.md) 의 코드 구조. [Ch 3 노트북 예산](../part1/03-laptop-budget.md) 의 메모리 식 (14N) 을 본 챕터에서 분해.
...
- **Part 4 Ch 13** — mixed precision · grad accumulation 으로 메모리 1/2~1/4 줄이기
- **Part 6** — int4 양자화로 추론 메모리 1/4
- **Part 7 Ch 23** — 노트북에서 가능한 LoRA 베이스 크기 결정
...
**weight tying** (Ch 10) 면 `lm_head` 가 같은 가중치 사용 → 2배 안 듦.
...
![학습 메모리 분해 — 항별 비율](../assets/diagrams/memory-breakdown-pct.svg#only-light)
![학습 메모리 분해 — 항별 비율](../assets/diagrams/memory-breakdown-pct-dark.svg#only-dark)
...
activation 을 모두 저장하지 않고 backward 시 다시 계산. 메모리 1/√L 수준 (예: 840MB → 350MB), 시간 1.3× 정도 더. Part 4 Ch 13.
...
- [ ] B 또는 T 를 줄여서 다시 산수
- [ ] grad accumulation 으로 effective batch 보정 가능 (Part 4 Ch 13)
...
| 챕터 | 무엇을 |
|---|---|
| Ch 8 | Attention — 식 한 줄, 코드 5 줄 |
| Ch 9 | RoPE · RMSNorm · SwiGLU · GQA |
| Ch 10 | nanoGPT 100 줄 — 모델 한 파일 |
| **Ch 11** | **파라미터 · 메모리 산수** |

다음 단계 → [Part 4 노트북에서 훈련](../part4/12-training-loop.md). 만든 모델을 굴리는 차례.
