...
!!! quote "전제"
    [Ch 8 Attention](08-attention.md) 의 SDPA, [Ch 9](09-modern-blocks.md) 의 RoPE/RMSNorm 개념. PyTorch `nn.Module` 한 번 만들어본 적.
...
1. RoPE 적용은 attention 직전, head 차원 분리 후. Ch 9 식 그대로.
2. multi-head 분리 → RoPE → SDPA. 한 줄에 FlashAttention 까지.
3. **pre-norm**. norm → sublayer → residual 두 번. Ch 9 의 권장.
